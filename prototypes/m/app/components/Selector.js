import React from "react"
import classNames from "classnames"
import letterActions from "../actions/LetterActions.js"
let ReactPropTypes = React.PropTypes

let Selector = React.createClass({

  propTypes: {
    areAllChecked: ReactPropTypes.bool,
  },

  getInitialState: function() {
    return {
        isCheckboxNavOpened : false
    }
  },

  render: function() {
    let selectorClassnames = classNames({
      'selector__checkbox': true,
      'selector__checkbox_checked': this.props.areAllChecked,
    });

    let selectorNavClassnames = classNames({
      'selector__nav': true,
      'selector__nav_opened': this.state.isCheckboxNavOpened,
    });

    return (
      <div className="selector">
        <div className={selectorClassnames} onClick={this._onToggleCheckAll}><svg viewBox="0 0 9 7" className="selector__checkbox-tick"><path d="M3.18937182,6.75830078 L0,3.46309132 L1.01985037,2.44324095 L3.18937178,4.80468826 L7.99406007,0 L9.05960657,1.06554661 L3.18937182,6.75830078 Z" ></path></svg></div>
        <div className="selector__bird" onClick={this._toggleCheckboxNavOpened}></div>
        <ul className={selectorNavClassnames}>
          <li className="selector__nav-item"><a href="#">Выбрать все</a></li>
          <li className="selector__nav-item"><a href="#">Непрочитанные</a></li>
          <li className="selector__nav-item"><a href="#">Прочитанные</a></li>
        </ul>
      </div>
    )
  },

  _toggleCheckboxNavOpened: function() {
    this.setState({
      isCheckboxNavOpened: this.state.isCheckboxNavOpened ? false : true,
    });
  },

  _onToggleCheckAll: function() {
    letterActions.toggleCheckAll();
  },

});

module.exports = Selector;
