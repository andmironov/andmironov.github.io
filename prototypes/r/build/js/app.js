...
if(t!==1){this.lastY=window.pageYOffset;window.requestAnimationFrame(this.step.bind(this));}else{if(this.onFinish)this.onFinish();}};module.exports=animateScroll;},{}],2:[function(require,module,exports){var forEach=require('lodash.forEach'),extendObject=require('lodash.assign');function Caroucel(options){options=extendObject(Caroucel.options,options);this.items=options.items;this.wrap=options.wrap;this.button=options.button;this.currentClassname="caroucel__item--current";this.currentIndex=0;this.arrowLeft=document.querySelectorAll(".caroucel__arrow--left")[0],this.arrowRight=document.querySelectorAll(".caroucel__arrow--right")[0];}Caroucel.prototype={constructor:Caroucel,init:function init(){//hack
var self=this;this.update(this.currentIndex);this.arrowLeft.addEventListener("click",function(event){event.preventDefault();self.moveLeft();});this.arrowRight.addEventListener("click",function(event){event.preventDefault();self.moveRight();});this.button.addEventListener("click",function(event){event.preventDefault();self.moveRight();});},update:function update(newIndex,direction){var oldIndex=this.currentIndex;this.updateItems(newIndex,direction,oldIndex);this.updateNav(newIndex,direction,oldIndex);this.updateWrap(newIndex);this.currentIndex=newIndex;return;},updateItems:function updateItems(newIndex,direction,oldIndex){var classname=this.currentClassname;this.addTemporaryClassName(oldIndex,direction);forEach(this.items,function(el,i){i!==newIndex?el.classList.remove(classname):el.classList.add(classname);});},updateNav:function updateNav(newIndex){switch(newIndex){case this.items.length-1:this.disableArrow("right");this.enableArrow("left");break;case 0:this.disableArrow("left");this.enableArrow("right");break;default:this.enableArrow("all");}},moveLeft:function moveLeft(){if(this.currentIndex>0){var newIndex=this.currentIndex-1;this.update(newIndex,"left");}},moveRight:function moveRight(){if(this.currentIndex<this.items.length-1){var newIndex=this.currentIndex+1;this.update(newIndex,"right");}},disableArrow:function disableArrow(arrow){switch(arrow){case"left":this.arrowLeft.classList.add("disabled");break;case"right":this.arrowRight.classList.add("disabled");break;case"all":this.arrowLeft.classList.add("disabled");this.arrowRight.classList.add("disabled");break;}},enableArrow:function enableArrow(arrow){switch(arrow){case"left":this.arrowLeft.classList.remove("disabled");break;case"right":this.arrowRight.classList.remove("disabled");break;case"all":this.arrowLeft.classList.remove("disabled");this.arrowRight.classList.remove("disabled");break;}},addTemporaryClassName:function addTemporaryClassName(index,direction){var item=this.items[index];switch(direction){case"right":item.classList.add("moving-left");setTimeout(function(){item.classList.remove("moving-left");},500);break;case"left":item.classList.add("moving-right");setTimeout(function(){item.classList.remove("moving-right");},500);break;}},inverseDirection:function inverseDirection(direction){var inversedDirection=void 0;switch(direction){case"right":inversedDirection="left";break;case"left":inversedDirection="right";break;}return inversedDirection;},updateWrap:function updateWrap(newIndex){newIndex==0?this.wrap.classList.add("slide--blue"):this.wrap.classList.remove("slide--blue");}};Caroucel.options={items:document.querySelectorAll(".caroucel__item"),wrap:document.querySelectorAll(".slide--four")[0],button:document.querySelectorAll(".button--gallery")[0]};module.exports=Caroucel;},{"lodash.assign":6,"lodash.forEach":8}],3:[function(require,module,exports){window.requestAnimationFrame=window.requestAnimationFrame||window.webkitRequestAnimationFrame||window.mozRequestAnimationFrame;/**
 ...
 */function Debouncer(callback){this.callback=callback;this.ticking=false;}Debouncer.prototype={constructor:Debouncer,/**
   ...
   */handleEvent:function handleEvent(){this.requestTick();}};module.exports=Debouncer;},{}],4:[function(require,module,exports){var isEqual=require('lodash.isequal'),forEach=require('lodash.foreach'),isEmpty=require('lodash.isempty'),cloneObject=require('lodash.clone'),extendObject=require('lodash.assign'),debouncer=require("./Debouncer");function Scrllr(options){options=extendObject(Scrllr.options,options);this.lastKnownScrollY=0;this.initialised=false;this.onScrollCallback=options.onScrollCallback;}Scrllr.prototype={constructor:Scrllr,init:function init(){this.debouncer=new debouncer(this.update.bind(this));// defer event registration to handle browser
// potentially restoring previous scroll position
setTimeout(this.attachEvent.bind(this),100);return this;},attachEvent:function attachEvent(){if(!this.initialised){this.lastKnownScrollY=this.getScrollY();this.initialised=true;window.addEventListener('scroll',this.debouncer,false);this.debouncer.handleEvent();}},getScrollY:function getScrollY(){return window.pageYOffset!==undefined?window.pageYOffset:window.scrollTop!==undefined?window.scrollTop:(document.documentElement||document.body.parentNode||document.body).scrollTop;},update:function update(){var currentScrollY=this.getScrollY(),scrollDirection=currentScrollY>this.lastKnownScrollY?'down':'up';this.onScrollCallback(currentScrollY);this.lastKnownScrollY=currentScrollY;},destroy:function destroy(){this.initialised=false;window.removeEventListener('scroll',this.debouncer,false);}};Scrllr.options={onScrollCallback:function onScrollCallback(){}};module.exports=Scrllr;},{"./Debouncer":3,"lodash.assign":6,"lodash.clone":7,"lodash.foreach":9,"lodash.isempty":10,"lodash.isequal":11}],5:[function(require,module,exports){var Caroucel=require('./caroucel.js'),forEach=require('lodash.foreach');//Caroucel
var caroucel=new Caroucel();caroucel.init();//Slides
var Scrllr=require("./lib/Scrllr.js"),animateScroll=require('./animatescroll.js'),slides=document.querySelectorAll(".slide"),nav=document.querySelectorAll(".header .nav a"),viewportHeight=getViewportHeight(),elements=new Array();var scrllr=new Scrllr({onScrollCallback:cb});scrllr.init();function getElementProperties(el,scrollY){var rect=el.getBoundingClientRect();return{offsetTop:rect.top,height:rect.height,scrollY:rect.top+scrollY};}function getViewportHeight(){return Math.max(document.documentElement.clientHeight,window.innerHeight||0);}function cb(currentScrollY){forEach(slides,function(el,i){elements[i]=getElementProperties(el,currentScrollY);if(elements[i].offsetTop<elements[i].height/2&&elements[i].offsetTop>-Math.abs(elements[i].height/2))update(i,currentScrollY);});}function initNav(){forEach(nav,function(el,i){var targetElement=slides[i+1];nav[i].addEventListener("click",createHandler(targetElement,event));});}function createHandler(targetElement){return function(){animateScroll(targetElement,600,"easeInOutCubic",0);};}document.querySelectorAll(".button--scroller")[0].addEventListener('click',function(event){animateScroll(slides[1],600,"easeInOutCubic",0);event.preventDefault();});document.querySelectorAll(".slide__scroller")[0].addEventListener('click',function(event){animateScroll(slides[1],600,"easeInOutCubic",0);event.preventDefault();});function update(index,scrollY){showSlide(index);initNav();updateNav(index);updateHeader(index,scrollY);}function showSlide(index){updateNav(index);var classname="slide--active";slides[index].classList.add(classname);}function updateNav(index){index--;var classname="nav__item--active";forEach(nav,function(el,i){i!==index?el.classList.remove(classname):el.classList.add(classname);});}function updateHeader(index,scrollY){var el=document.querySelectorAll(".header")[0],classname="header--scrolled";scrollY>300?el.classList.add(classname):el.classList.remove(classname);}//Oh dear god
document.querySelectorAll(".button--description")[0].addEventListener('click',function(e){document.querySelectorAll(".slide--two .hidden-text")[0].classList.remove("hidden-text");document.querySelectorAll(".button--description")[0].classList.add("button--hidden");e.preventDefault();});document.querySelectorAll(".button--prediction")[0].addEventListener('click',function(e){document.querySelectorAll(".slide--three .hidden-text")[0].classList.remove("hidden-text");document.querySelectorAll(".button--prediction")[0].classList.add("button--hidden");e.preventDefault();});document.querySelectorAll(".button--astronomy")[0].addEventListener('click',function(e){document.querySelectorAll(".slide--five .hidden-text")[0].classList.remove("hidden-text");document.querySelectorAll(".button--astronomy")[0].classList.add("button--hidden");e.preventDefault();});},{"./animatescroll.js":1,"./caroucel.js":2,"./lib/Scrllr.js":4,"lodash.foreach":9}],6:[function(require,module,exports){/**
 ...
 * _.keys('hi');
 * // => ['0', '1']
 */function keys(object){return isArrayLike(object)?arrayLikeKeys(object):baseKeys(object);}module.exports=assign;},{}],7:[function(require,module,exports){(function(global){/**
 * lodash (Custom Build) <https://lodash.com/>
 * Build: `lodash modularize exports="npm" -o ./`
 * Copyright jQuery Foundation and other contributors <https://jquery.org/>
 * Released under MIT license <https://lodash.com/license>
 ...
 * // => [false, false]
 */function stubFalse(){return false;}module.exports=clone;}).call(this,typeof global!=="undefined"?global:typeof self!=="undefined"?self:typeof window!=="undefined"?window:{});},{}],8:[function(require,module,exports){/**
 * lodash (Custom Build) <https://lodash.com/>
 * Build: `lodash modularize exports="npm" -o ./`
 * Copyright jQuery Foundation and other contributors <https://jquery.org/>
 * Released under MIT license <https://lodash.com/license>
 ...
 * // => true
 */function identity(value){return value;}module.exports=forEach;},{}],9:[function(require,module,exports){arguments[4][8][0].apply(exports,arguments);},{"dup":8}],10:[function(require,module,exports){(function(global){/**
 * lodash (Custom Build) <https://lodash.com/>
 * Build: `lodash modularize exports="npm" -o ./`
 * Copyright jQuery Foundation and other contributors <https://jquery.org/>
 * Released under MIT license <https://lodash.com/license>
 ...
 * _.times(2, _.stubFalse);
 * // => [false, false]
 */function stubFalse(){return false;}module.exports=isEmpty;}).call(this,typeof global!=="undefined"?global:typeof self!=="undefined"?self:typeof window!=="undefined"?window:{});},{}],11:[function(require,module,exports){(function(global){/**
 * lodash (Custom Build) <https://lodash.com/>
 * Build: `lodash modularize exports="npm" -o ./`
 * Copyright jQuery Foundation and other contributors <https://jquery.org/>
 * Released under MIT license <https://lodash.com/license>
 ...
 * _.keys('hi');
 * // => ['0', '1']
 */function keys(object){return isArrayLike(object)?arrayLikeKeys(object):baseKeys(object);}module.exports=isEqual;}).call(this,typeof global!=="undefined"?global:typeof self!=="undefined"?self:typeof window!=="undefined"?window:{});},{}]},{},[5]);
